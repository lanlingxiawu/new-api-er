...
② 后台导出（独立 goroutine，与 HTTP 请求解耦）
   for 时间窗口 (start → end, 每窗 1h):
     for keyset 分页 (created_at asc, id asc):
...
| 情况 | 处理 |
|---|---|
| 导出期间新日志写入 | 时间窗口的上界在创建任务时固定，晚于它的新日志不在范围内；结果是「发起时刻之前」的快照。扫描**由早到晚**推进，日志异步批量落库时尚未写入的近期行仍在游标前方，等扫到时已经落库，比逆序更不容易漏 |
