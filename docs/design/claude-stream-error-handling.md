...
以下历史实现记录中的连接失败、非 200 诊断已由 [成功响应门控](stream-success-response-gate.md) 覆盖：流式仅真实 HTTP 200 / 成功 WS 101 后启用新流程；未收到 HTTP 响应和非 200 沿用原错误、重试、退款，不采集或保存新诊断。已移除原生 Claude 将连接错误伪装为响应读取错误的终止入口。非流式旧采集行为不变。

## 全渠道流式扩展（2026-09-12）

后续实现见 [全渠道方案](unified-stream-error-handling.md)：异常计费合同保持一致，新增公共入口/协议观察和媒体、SDK、WS 接入；非流式保持旧流程。通用结果已改名 `StreamOutcome`、`stream_result`、`stream_diagnostic`，旧 JSON 名称仅读取兼容；后续下游正文记录见 [下游正文方案](stream-diagnostic-body.md)。原生 Claude 严格状态机仍独立运行，与通用终止所有者互斥。
...
The retained response bytes, low-level error and usage evidence are private to Root. General `other` includes terminal/billing summaries, `stream_diagnostic_attempt` and the confirmed-upstream-failure flag `stream_diagnostic_available`. As explicitly approved on 2026-09-13, `reject_reason` follows bb6317462 again: a separate top-level field retained by ordinary APIs and displayed only to admins in the UI. A dedicated RootAuth GET `/api/log/claude-diagnostic?request_id=...&created_at=...&attempt=...` selects at most 128 same-request/time rows and matches the attempt (zero for historical records). It returns `success/data`, uses `Cache-Control: no-store`, and audits access without payload. Ordinary reads strip the private envelope, project only historical nested policy reasons, and do not infer availability from old markers or stored bodies. Existing request-log raw-detail access stays Root-only. UI loads diagnostics on demand with account/attempt-scoped keys, zero cache retention after unmount, and no persisted diagnostic cache. See `stream-diagnostic-visibility.md` for the current visibility contract.
...
- Root selection includes the attempt, including same-request/same-second retries. Root/user/admin endpoint fixtures and invalid selectors are covered. Policy reason visibility is superseded by the approved bb6317462 restoration in `stream-diagnostic-visibility.md`; raw response evidence remains private. Legacy Claude stream error details stay out of public stream-status fields. Console/error-log summaries do not include raw upstream causes on the native Messages path.
