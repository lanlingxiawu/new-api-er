...
把请求日志（relay 请求的下游请求头/请求体、返回头/返回体）的存储从 Redis 迁移为：
...
同时解决两个既有问题：写入路径的文件句柄/连接占用必须与请求量解耦（第 5 节）；`use_time` 字段从未被赋值（第 10 节）。
...
**明确的非目标**：不保证进程崩溃（SIGKILL / 断电）时的数据完整性。请求日志是排障用的尽力而为可观测性数据，不参与计费与审计，允许丢失。
...
relay 请求结束
  └─ middleware（同步，relay goroutine）
       ...
       └─ 非阻塞投递到 requestLogQueue（容量 1000），满则丢弃 + 限流告警
            └─ writer 池（固定 4 个常驻 goroutine）
                 ├─ 1. 分配自增 id（进程内 atomic，不再走 Redis INCR）
                 ├─ 2. 写磁盘文件 relay_log/<date>/<ts%8>/<ts>_<request_id>.json（完整 JSON）
                 └─ 3. 写成功后才把 meta 追加进内存索引；超过 max 时截断到 min（不删文件）
...
每 5 分钟    → 扫描 relay_log → 文件不在内存索引中且超过宽限期 → 删除；空的历史日期目录整体删除
```

**磁盘写在内存索引之前**：保证"索引 ⊆ 磁盘"恒成立，详情接口永远不会拿到一个指向缺失文件的索引。反过来（先写索引再写文件）会让扫描协程在两步之间把刚写的文件当孤儿，或让详情读到 404。写盘失败则整条丢弃（限流日志），不产生索引。
...
- **文件内容**：一条完整 `RequestLog` 的 JSON（含大字段），`common.Marshal` 产出，一次写入（`0644`）。不调用 `fsync`——崩溃丢失可接受（第 1 节非目标）。
...
- **未完成量记账**：`pending` 计数在**投递成功前** +1、worker 处理完 -1，被丢弃时回退。不能改成"worker 收到任务后再 +1"——出队与自增之间存在空隙，关停排空会在那一刻看到"队列空且无在途"而提前返回，导致最后几条来不及进索引就被快照落下。实现期间的测试正是先按后者写、被这个竞态打出来的。
...
存 `rel` 而不是每次由 `created_at`/`request_id` 重新推导，是为了让"索引 ↔ 文件"的对应关系只有一处真值：净化规则或目录分层将来再调整，也不会让老条目失联。
...
淘汰阈值沿用现有 `effectiveRequestLogLimits()`（保证 `0 <= min < max`，配置错配时退化为 `max/2`，杜绝"永不清理"）。**淘汰只删索引，不碰磁盘文件**——磁盘回收统一交给第 8 节的扫描协程，写入路径因此不含任何删除 IO。
...
- `ClearAllRequestLogs()`：清空索引并返回条数，**异步**触发一次立即扫描（已确认采用异步方案），磁盘在秒级内回收而不阻塞接口，也不必等下一个 5 分钟周期。
...
- **Redis 未启用**：快照与恢复都是 no-op。重启后索引为空，磁盘旧文件会在超过宽限期后被扫描协程删除——这正是需求语义，需在设置页文案说明"未启用 Redis 时重启会丢失请求日志"。
...
进程启动（且索引恢复完成）后启动一个 goroutine，`time.Ticker` 每 5 分钟触发一次：

1. 持锁构建当前索引中所有 `rel` 的 `map[string]struct{}`（5000 条约 200 KB，用完即弃），同时取出索引中最早的日期。
2. `ReadDir` 根目录得到日期层。**日期目录名可解析且早于 `今天-1` 天、且索引中没有任何该日期前缀的条目** → 直接 `os.RemoveAll` 整个日期目录，不逐文件 `Stat`。这是长期停机后重启（磁盘上堆积远超 `MaxCount` 个文件）的关键优化：`ReadDir` 在多数平台不返回 mtime，逐文件 `Info()` 就是逐文件 `stat`，整目录删除把这部分开销一次性省掉。安全性来自"2 天前的目录里不可能有落在 10 分钟宽限期内的文件"，除非系统时钟跳变超过一天——超出本功能范围。
3. 其余日期目录逐桶（`0`–`7`）用 `dir.ReadDir(1000)` **分批**读取目录项，拼成 `<date>/<bucket>/<name>` 与集合比对。
4. 不在集合中的文件：取 `ModTime()`，**超过宽限期**（默认 600 秒）才删除。宽限期解决两类竞态：写盘成功但尚未插入索引的瞬间窗口；并发写入与扫描的交叉。
5. 名字不符合 `<数字>_<id>.json` 的文件、以及根目录下不符合日期格式的条目，同样按孤儿处理，仍受宽限期保护（本目录归本功能独占，独占性由 §8.1 的归属校验保证）。根目录下的归属标记文件 `.new-api-request-log` 是唯一豁免项——删掉它下次启动就无法再证明目录独占。
6. 处理完一个日期目录后，若其下桶目录已空且日期早于今天，`os.Remove` 掉空目录，避免空壳目录无限累积。
7. 每轮结束记一条汇总日志（扫描文件数、删除数、整目录删除数、耗时）；单文件删除失败只累计计数，不中断本轮。整个循环 `recover()` 兜底。

参数：`REQUEST_LOG_SWEEP_INTERVAL_SEC`（默认 300，设为 0 关闭扫描）、`REQUEST_LOG_SWEEP_GRACE_SEC`（默认 600）。宽限期有 60 秒硬下限：它保护的是"写盘成功但尚未进索引"的瞬间窗口，配成 0 会让扫描协程删掉在途正文，详情点开即 404；关闭清理请用 `INTERVAL=0` 表达。扫描期间同时最多打开 1 个目录句柄。
...
| 插入内存索引 + 淘汰 | writer 池 | 新增，持锁 O(1) 摊还 |
| Redis 写 | — | **移除** |
...
| **文件句柄** | 写入峰值 = writer 数（4），扫描期 1 个目录句柄 | relay 大量占用上游连接 fd | **与 RPM 解耦**，恒定 ≤5，见第 5 节 |
...
| 内存 `reqLogItems` + `reqLogMu` | writer 写、管理端读 | 否（relay goroutine 从不持这把锁） | 隔离 |
...
- **锁竞争**：`reqLogMu` 每条日志一次 O(1) 操作，1667 次/秒下最多 4 个 writer 争用；管理端列表持锁只做一次切片复制。
- **磁盘容量**：稳态文件数受 `MaxCount` 约束（扫描协程回收），不随 RPM 增长。峰值上限 ≈ `MaxCount × 单文件上限` 加上一个扫描周期（5 min）内的新增量。
...
| `REQUEST_LOG_SWEEP_INTERVAL_SEC` | `300` | 扫描周期，0 关闭 |
| `REQUEST_LOG_SWEEP_GRACE_SEC` | `600` | 孤儿文件删除宽限期，硬下限 60 秒 |
...
| 场景 | 处理 |
|---|---|
| 根目录创建失败（启动时） | `common.SysError` 记录并把功能整体标记为不可用，投递路径直接返回。不降级为"只存索引不存正文"——那会产生点不到正文的索引条目，比没有日志更误导 |
| 单条写盘失败 | 不插索引，按 1/1000 频率 `common.SysError` 限流打印（与丢弃告警一致），不影响响应 |
| 序列化失败 | 同上 |
| 详情读文件失败/文件缺失 | 返回 `errRequestLogNotFound`，控制器 `common.ApiError` → HTTP 200 + `success:false`；前端提示"该日志正文已被清理" |
...
1. **新增耗时列**（第 10 节），`types.ts` 的 `use_time` 改为 `use_time_ms`。
2. **文案修正**（4 处，均因存储介质变化）：
...
| `Clear all request logs` 描述 | "移除 Redis 中存储的全部请求日志" | "清空内存中的请求日志索引，对应正文文件将在后台扫描时删除" |
| 清空确认弹窗描述 | 同上 | 同上 |
...
**写入与索引**
- 写盘成功 → 文件存在、内容可反序列化回原结构（含 `use_time_ms`）、索引 +1、索引条目大字段为空
...
**扫描**（决策覆盖）
- 有索引的文件保留；无索引且超宽限期的删除；无索引但在宽限期内的保留
- 非法文件名、根目录下非日期条目 + 超宽限期 → 删除
- 归属标记文件即使远超宽限期也保留
- 根目录归属：含无关内容 → 功能不启用且无关文件不被触碰；仅含日期目录 → 认领并补写标记
- 宽限期下限：环境变量填 `-1` / `0` 都被抬到 60 秒，高于下限的值原样保留
- 早于 `今天-1` 且索引中无该日期条目的日期目录 → 整目录删除，且**不**逐文件 stat（用只读文件构造"逐个删会失败"的场景来锁住走的是 `RemoveAll` 分支）
- 早于今天但索引中仍有该日期条目 → 逐文件判定，索引内文件保留
- 空桶目录/空日期目录被回收；根目录不存在、空目录 → 不 panic
- 单桶 >1000 个文件 → 分批读取全部覆盖
- 扫描与写入并发 → 无 panic，新写入文件不被误删
...
| `model/request_log.go` | 内存索引容器（尾部追加 + 淘汰）、`RecordRequestLog`（写盘 → 插索引）、查询/详情/按时间删除/清空 |
| `model/request_log_snapshot.go` | 快照保存/恢复、遗留 Redis 键一次性清理 |
| `model/request_log_sweep.go` | 清理协程、`SweepRequestLogFiles`、整目录删除与空目录回收 |
| `middleware/request_logger.go` | 队列 + writer 池、`DrainRequestLogQueue`、`defer` 捕获、`use_time_ms` 计时、用户名异步解析、头部截断 |
...
| `controller/request_log.go`、`i18n/` | `ApiErrorI18n` + `request_log.not_found` / `request_log.timestamp_required` 三语 |
...
1. `ClearAllRequestLogs` 采用**异步**回收：清空索引后触发一次立即扫描，接口不等磁盘删除完成。
2. 磁盘**加日期目录**层：`relay_log/<YYYY-MM-DD>/<ts%8>/`。除按需求分桶外，还让长期停机后的清理可以整目录删除，并支持将来按天做保留策略。
3. `use_time` **补上**，改为毫秒并重命名 `use_time_ms`，前端新增耗时列。
4. 文件句柄与连接数与请求量解耦：固定 4 个 writer（峰值 fd = 4），Redis 稳态零流量、独立客户端池由 10 降至 2。
5. **请求/返回头不做脱敏**（见第 17 节）。
6. **新增环境变量一律不在包级变量里读**（见第 11 节末）。
...
| `ClearAllRequestLogs` 的 `SCAN MATCH` 在默认配置（`RequestLogRDB == RDB`）下遍历**主库全键空间** | 同上 |
...
实现期间新发现并修复的一条：**关停排空的竞态**（见第 5 节末）。写队列的未完成量若由 worker 在出队后自增，排空会在"已出队、尚未记账"的空隙里误判为已完成，最后几条日志来不及进索引就被快照落下。改为投递侧记账。
