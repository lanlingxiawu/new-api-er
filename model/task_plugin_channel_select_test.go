...
func TestTaskPluginChannelSelectionFiltersBothCachePaths(t *testing.T) {
	cleanupTaskPluginSelectChannels(t, 900001, 900002, 900003, 900004, 900005)
	priority := int64(0)
	weight := uint(1)
...
func TestSharedPluginKeysFilterBothChannelSources(t *testing.T) {
	cleanupTaskPluginSelectChannels(t, 910001, 910002, 910003)
...
// cleanupTaskPluginSelectChannels is the fork's row-scoped replacement for
// upstream's global truncateTables: it removes only the fixed test channel ids
// (and their abilities) before and after the test.
func cleanupTaskPluginSelectChannels(t *testing.T, ids ...int) {
	t.Helper()
	requireDB(t)
	purge := func() {
		DB.Where("channel_id IN ?", ids).Delete(&Ability{})
		DB.Where("id IN ?", ids).Delete(&Channel{})
	}
	purge()
	t.Cleanup(purge)
}
