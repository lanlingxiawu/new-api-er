...
func TestPricingCarriesTaskUsageSchemaAndRefreshesWithPluginGeneration(t *testing.T) {
  ...
  action: {enum:["video"],enumLabels:{video:{en:"Generate video",zh:"生成视频"}}}
	...
	insertPricingEndpointChannel(t, pricingTestChannelID(901), constant.ChannelTypeTaskPlugin, dto.ChannelOtherSettings{})
	insertPricingEndpointAbility(t, pricingTestChannelID(901), "pricing-usage-model")
	insertPricingEndpointAbility(t, pricingTestChannelID(901), "ordinary-model")
...
func TestPricingAliasCarriesPluginUsageSchemaAndTailExpr(t *testing.T) {
  ...
  seconds: {type: "number", unit: "second", description: "Estimated duration."}
	...
	insertPricingEndpointAbility(t, pricingTestChannelID(910), "alias-model")
	insertPricingEndpointAbility(t, pricingTestChannelID(910), "pricing-usage-model")
	InitChannelCache()
	...
	insertPricingEndpointAbility(t, pricingTestChannelID(911), "alias-own-expr")
	InitChannelCache()
	InvalidatePricingCache()
...
`
	...
	for index, mapping := range []string{
		...
		id := pricingTestChannelID(920 + index)
...
func TestPricingSharedPluginVariantsUseEachSchemaAndExpression(t *testing.T) {
	...
	insertPricingEndpointChannel(t, pricingTestChannelID(930), constant.ChannelTypeTaskPlugin, dto.ChannelOtherSettings{})
	insertPricingEndpointAbility(t, pricingTestChannelID(930), "pricing-usage-model")
