...
// Cases from upstream/main:model/token_auto_groups_cache_test.go, run on the
// fork's shared DB harness: tokens use unique keys / user ids and are deleted
// by primary key instead of truncating the tokens table.
...
func TestTokenUpdateSynchronouslyNarrowsPreheatedAutoGroupsCache(t *testing.T) {
	...
	tokenID := token.Id
	t.Cleanup(func() { DB.Unscoped().Delete(&Token{}, tokenID) })
	require.NoError(t, cacheSetTokenForTest(token))
