...
// Cases from upstream/main:model/user_authentication_test.go, run on the
// fork's shared DB harness. Upstream truncates every auth table and uses fixed
// ids / literal unique values; here users come from mkUser, every value behind
// a unique index (token key, credential id, session id, flow hash, OAuth and
// external identity subjects) is generated per test, and cleanup removes only
// the rows owned by the test user.

// userAuthCasesUser inserts a user and registers cleanup for every
// authentication table row keyed by that user.
func userAuthCasesUser(t *testing.T, mut func(u *User)) User {
	t.Helper()
	user := mkUser(t, mut)
	userID := user.Id
	t.Cleanup(func() {
		for _, record := range []any{
			...
			DB.Unscoped().Where("user_id = ?", userID).Delete(record)
		}
	})
	return *user
}

// seedUserAuthenticationData attaches one row of every authentication table to
// the user so hard-delete purge / fail-closed behaviour can be observed.
func seedUserAuthenticationData(t *testing.T, user User) {
	t.Helper()
	require.NoError(t, DB.Transaction(func(tx *gorm.DB) error {
		return ClaimExternalIdentityWithTx(tx, ExternalIdentityProviderTelegram, user.TelegramId, user.Id)
	}))
	require.NoError(t, DB.Create(&Token{UserId: user.Id, Key: uniq("hdtok")}).Error)
	...
	require.NoError(t, DB.Create(&PasskeyCredential{UserID: user.Id, CredentialID: uniq("credential"), PublicKey: "public-key"}).Error)
	require.NoError(t, DB.Create(&UserOAuthBinding{UserId: user.Id, ProviderId: 1, ProviderUserId: uniq("provider-user")}).Error)
	require.NoError(t, DB.Create(&UserSession{
		SID: uniq("hard-delete-session"), UserID: user.Id, Version: 1, UserAuthVersion: 1,
	...
	require.NoError(t, DB.Create(&AuthFlow{
		TokenHash: uniq("hard-delete-auth-flow"), Purpose: AuthFlowPurposeTwoFALogin,
...
func TestHardDeleteUserFailsClosedWhenAuthFenceCannotPublish(t *testing.T) {
	user := userAuthCasesUser(t, func(u *User) { u.TelegramId = uniq("hard-delete-telegram") })
	seedUserAuthenticationData(t, user)
...
func TestHardDeleteUserPublishesTombstoneAndPurgesAuthenticationData(t *testing.T) {
	user := userAuthCasesUser(t, func(u *User) {
		u.AuthVersion = 1
		u.TelegramId = uniq("hard-delete-success-telegram")
	})
...
func TestValidateBackupCodeCanOnlySucceedOnce(t *testing.T) {
	...
	user := userAuthCasesUser(t, func(u *User) { u.AuthVersion = 1 })
...
func TestPendingTwoFASetupAPIsRejectEnabledFactor(t *testing.T) {
	...
	session := UserSession{SID: uniq("enabled-factor-session"), UserID: user.Id, Version: 1, UserAuthVersion: 1, Status: UserSessionStatusActive, ExpiresAt: time.Now().Add(time.Hour).Unix()}
...
func TestSecurityFactorMutationsAdvanceUserAuthVersion(t *testing.T) {
	user := userAuthCasesUser(t, func(u *User) { u.AuthVersion = 1 })
	session := UserSession{SID: uniq("factor-version-session"), UserID: user.Id, Version: 1, UserAuthVersion: 1, Status: UserSessionStatusActive, ExpiresAt: time.Now().Add(time.Hour).Unix()}
	require.NoError(t, DB.Create(&session).Error)
	identity := AuthSessionIdentity{UserID: user.Id, SessionID: session.SID, UserAuthVersion: 1, SessionVersion: 1}
	authorization := &AuthFlowAuthorization{AuthSessionIdentity: identity, ProofID: 1, Scope: "2fa.setup", ContextHash: "setup-context", Method: "password"}
	...
	credential := &PasskeyCredential{UserID: user.Id, CredentialID: uniq("credential-id"), PublicKey: "public-key"}
...
func TestUpdatePasskeyAssertionStateCannotRewriteRegistrationIdentity(t *testing.T) {
	...
	t.Cleanup(func() {
		system_setting.ReplacePasskeySettings(previousSettings)
	...
	// The fork publishes settings as an immutable snapshot; replace, never write through the pointer.
	system_setting.ReplacePasskeySettings(system_setting.PasskeySettings{RPID: "example.com", Origins: "https://example.com"})
	...
	user := userAuthCasesUser(t, func(u *User) { u.AuthVersion = 1 })
	credentialID := []byte(uniq("stable-credential-id"))
	...
	validated.ID = []byte(uniq("another-credential"))
