...
function getLogMeta<TData>(cell?: Cell<TData, unknown>) {
  return cell?.row.original as
    | {
        created_at?: unknown
        type?: unknown
      }
...
function CommonLogsCard<TData>({
  ...
  const timeMeta = getLogMeta(cells.get('created_at'))
