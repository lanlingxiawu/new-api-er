...
type NormalizedRequiredTextPart = RequiredTextPart & {
  inputIndex?: number
  /** Render key: part type plus its position in requiredTextParts. */
  renderKey: string
}
...
export function RiskAcknowledgementDialog({
  ...
  >(() => {
    return requiredTextParts.reduce<{
      ...
      (acc, part) => {
        const renderKey = `${part.type}-${acc.parts.length}`
        if (part.type !== 'input') {
          return { ...acc, parts: [...acc.parts, { ...part, renderKey }] }
        }
        return {
          parts: [
            ...acc.parts,
            { ...part, inputIndex: acc.inputIndex, renderKey },
          ],
