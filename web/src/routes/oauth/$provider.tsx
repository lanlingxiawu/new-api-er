...
import { api, applyAuthBundle, getSelf, isAuthBundle } from '@/lib/api'
    ...
    ;(async () => {
      ...
      try {
        ...
        if (res?.data?.success) {
          ...
          if (loginUser) {
            // OAuth 回调同样返回完整凭证；存下 token 才能带 Authorization 头。
            const payload = res.data?.data
            if (isAuthBundle(payload)) {
              applyAuthBundle(payload)
            } else {
              useAuthStore.getState().auth.setUser(loginUser)
            }
            try {
              const uid = loginUser.id ?? (payload as { user?: { id?: number } })?.user?.id
