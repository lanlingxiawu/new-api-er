...
const VERIDROP_SETTINGS_SCOPE = 'veridrop-detection'
...
export async function getVeridropOptions() {
  const res = await api.get<SystemOptionsResponse>('/api/option/', {
    params: { scope: VERIDROP_SETTINGS_SCOPE },
  })
...
export async function fetchManualUpstreamModels(
  ...
  const res = await api.post<{ success: boolean; data?: string[] }>(
    '/api/channel/veridrop/manual_models',
    {
...
export async function listVeridropSystemTasks(limit = 30) {
  const res = await api.get<SystemTaskListResponse>(
    '/api/channel/veridrop/tasks',
    { params: { limit }, disableDuplicate: true }
  )
  return res.data
}
