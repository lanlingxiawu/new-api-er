...
export function useAuthRedirect() {
  ...
  const handleLoginSuccess = async (
    ...
    // 登录/注册/OAuth 回调都会带回完整凭证（access token + 会话），
    // 必须先存下来：后续所有后台接口靠它带 Authorization 头，
    // 漏掉这一步会立刻在 getSelf 上 401。
    if (isAuthBundle(loginData)) {
      applyAuthBundle(loginData)
    }

    // Save user ID if available
    const userData = loginData as { id?: number; user?: { id?: number } } | null
