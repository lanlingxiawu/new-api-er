...
export function ModelMutateDrawer(props: {
  ...
  // 模型定价属于系统设置「模型定价」分区，无查看权限时不展示定价页签。
  ...
  const initialSection =
    props.initialSection === 'pricing' && !canViewModelPricing
      ? 'metadata'
  ...
  useEffect(() => {
    ...
    setPricingVisited(initialSection === 'pricing')
    setPricingName('')
        ...
        >
          ...
          >
            <TabsList className='grid w-full grid-cols-3 group-data-horizontal/tabs:h-auto'>
              ...
              <TabsTrigger
                value='pricing'
                disabled={!hasModelName || !canViewModelPricing}
                className='h-auto min-w-0 whitespace-normal'
              >
                {t('Pricing')}
              </TabsTrigger>
