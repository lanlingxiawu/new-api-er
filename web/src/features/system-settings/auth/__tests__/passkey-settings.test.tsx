...
// Saves without a domain-removal review still go through the settings save confirmation.
...
describe('Passkey website guidance', () => {
  ...
  it('disables filling while saving and enables it after the save finishes', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Save Changes' }))
    await confirmSave(user)
    // While saving, the confirmation dialog stays open and blocks the page.
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole('alertdialog', { name: 'Confirm Changes' })
      ).not.toBeInTheDocument()
    )
    await user.click(screen.getByRole('button', { name: 'Why set this?' }))
    expect(
      screen.getByRole('button', { name: 'Keep existing domain' })
    ).toBeEnabled()
  })
