...
export function PasskeySection(props: PasskeySectionProps) {
  ...
  const onSubmit = async (values: PasskeyFormValues) => {
    ...
    if (!changesDomains) {
      await requestSaveConfirmation(() => saveSettings(normalized))
      return
    }
    try {
      const result = await updateDomains.mutateAsync({
      ...
      await requestSaveConfirmation(() => saveSettings(normalized, preview))
    } catch {
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem data-settings-form-span='full' className='max-w-xl'>
                ...
                <FormControl>
                  <Input
                    placeholder={t('e.g. NEXAXIS API Console')}
                    value={field.value ?? ''}
