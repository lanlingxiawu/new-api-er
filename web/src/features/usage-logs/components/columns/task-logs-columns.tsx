...
function TaskCostCell({ log }: { log: TaskLog }) {
  ...
  // Tasks from the former Go adaptors carry pricing_* metadata; task plugin
  // tasks billed by usage expression carry the same dimensions in usage_facts.
  const facts = other?.usage_facts
  const duration = other?.pricing_duration_seconds ?? facts?.seconds
  const resolution =
    other?.pricing_resolution ?? facts?.output_resolution ?? facts?.resolution
  const imageCount = other?.pricing_reference_image_count ?? facts?.input_images
  const summaryParts = [duration ? `${duration}s` : '', resolution || '']
    .filter(Boolean)
    .map(String)
  if (imageCount && String(imageCount) !== '0') {
