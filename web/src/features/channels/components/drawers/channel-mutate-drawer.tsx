...
// 每日金额上限与账号余额查询两块位于「其他」分区；分区映射（lib/channel-configuration）
// 未登记这些字段，校验失败时需要在这里补齐定位。
const OTHER_SECTION_FORM_FIELDS = new Set<string>([
  'daily_quota_limit_amount',
  'daily_limit_recover_mode',
  'daily_limit_recover_minutes',
  'account_balance_url',
...
function getFormFieldConfigurationSection(
  field: string
): ChannelConfigurationSection {
  if (OTHER_SECTION_FORM_FIELDS.has(field)) return 'other'
  return getChannelConfigurationSection(field)
}
...
export function ChannelMutateDrawer({
  ...
  const {
    ...
    enabled: open && !showProviderPicker && canViewGroupPricing,
  })
  ...
  const { data: prefillGroupsData } = useQuery({
    ...
    enabled: open && !showProviderPicker && canViewModelsMenu,
  })
  ...
  const currentCostRatio = formValues.cost_ratio
  // 每日金额上限：未填上限时恢复方式控件置灰，避免用户在无效状态下调整却不生效。
  const dailyLimitRecoverMode =
    formValues.daily_limit_recover_mode ?? 'next_day'
  const dailyLimitEnabled = (formValues.daily_quota_limit_amount ?? 0) > 0
  ...
  // 每日金额上限与账号余额查询两块的状态在本地计算，并汇总进「其他」分区的状态。
  const getLocalBlockStatus = (
    configured: boolean,
    fields: (keyof ChannelFormValues)[]
  ): ChannelConfigurationStatus => {
    if (fields.some((field) => Boolean(formErrors[field]))) return 'error'
    return configured ? 'configured' : 'idle'
  }
  const dailyLimitStatus = getLocalBlockStatus(dailyLimitEnabled, [
    'daily_quota_limit_amount',
    'daily_limit_recover_mode',
    'daily_limit_recover_minutes',
  ])
  const accountBalanceStatus = getLocalBlockStatus(
    Boolean(
      formValues.account_balance_token?.trim() &&
      formValues.account_balance_user_id?.trim()
    ),
    ['account_balance_url', 'account_balance_token', 'account_balance_user_id']
  )
  const configurationSectionStatuses = { ...configuration.sections }
  for (const status of [dailyLimitStatus, accountBalanceStatus]) {
    if (
      status === 'error' ||
      (status === 'configured' && configurationSectionStatuses.other === 'idle')
    ) {
      configurationSectionStatuses.other = status
    }
  }
  ...
  useEffect(() => {
    // 关闭时的重置放在 handleOpenChangeComplete（退场动画结束后），
    // 关闭过程中抽屉保持原来的渠道和已填内容。
    if (!open) return
  ...
  const onInvalid: SubmitErrorHandler<ChannelFormValues> = useCallback(
    (errors) => {
      ...
      if (field) {
        setConfigurationSection(getFormFieldConfigurationSection(field))
  ...
  >(
    ...
    [onOpenChange, isSubmitting, showProviderPicker, providerTarget]
  )
    ...
    <div className='scroll-mt-4'>
      <ChannelBasicSection>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel required>{t('Cost Ratio')}</FormLabel>
              <FormControl>
                <Input
                  type='number'
                  step='0.01'
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='space-y-3'>
                  ...
                  {canViewGroupPricing && isGroupPricingError && (
                    <Alert>
                      <AlertDescription>
                        {t(
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {currentType !== CHANNEL_TYPE_TASK_PLUGIN && (
                      <FormDescription>
                        {t(
                          'Custom API base URL. For official channels, NEXAXIS API has built-in addresses. Only fill this for third-party proxy sites or special endpoints. Do not add /v1 or trailing slash.'
                        )}
                      </FormDescription>
                    )}
                    {currentType === CHANNEL_TYPE_TASK_PLUGIN &&
  ...
  // 金额管控自成一块：它决定渠道会不会被自动停用，属于风控开关，
  // 和「渠道额外设置」里那些请求行为开关不是一类东西，混在一起既难找也容易误改。
  const dailyLimitFields = (
    <div
      role='group'
      aria-label={t('Daily Amount Limit')}
      className={channelConfigurationBlockClassName(
        dailyLimitStatus,
        'space-y-4'
      )}
    >
      ...
      <fieldset
        ...
        className='space-y-4 disabled:opacity-60'
      >
        {/* 金额与恢复方式并排；选「N 分钟后恢复」时分钟数输入框另起一格。
            items-start 是必须的——两列描述文案长度不同，默认的 stretch 会让
            FormItem 高度不等，标签与控件就对不齐了。 */}
                ...
                <FormControl>
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      const normalized = normalizeLeadingZeros(e.target.value)
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>{dailyLimitRecoverDescription}</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
  ...
  const accountBalanceFields = (
    <div
      role='group'
      aria-label={t('Account Balance Query')}
      className={channelConfigurationBlockClassName(
  ...
  if (!isEditing && !providerTarget) {
    ...
    formContent = (
      <ChannelConfiguration
        ...
        statuses={configurationSectionStatuses}
        connection={
          <>
            {basicSection}
            {connectionSection}
