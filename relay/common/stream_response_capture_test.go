...
func TestClaudeCaptureRetryAndConcurrency(t *testing.T) {
	capture := NewStreamResponseCapture(3)
	...
	require.Empty(t, NewStreamResponseCapture(4).Snapshot().Error)
}
...
func TestClaudeCaptureTransportRetryDoesNotPoisonSuccess(t *testing.T) {
	...
	call := 0
	client := StreamDiagnosticHTTPClient{Capture: capture, Client: captureHTTPTestFunc(func(*http.Request) (*http.Response, error) {
		call++
		if call == 1 {
