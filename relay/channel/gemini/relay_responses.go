...
func GeminiResponsesStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	// 受管流式会话的 StreamStatus 仅在 [DONE] 时写终止原因，EOF 等结束由会话判定并由受管写入器过滤成功尾帧；
	// 此处只对未受管的旧路径按 StreamStatus 补发失败事件，避免受管正常结束被误判为异常。
	if !info.StreamSession.Active() && info.StreamStatus != nil && !info.StreamStatus.IsNormalEnd() {
