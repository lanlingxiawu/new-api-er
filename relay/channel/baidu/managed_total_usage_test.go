...
func TestManagedProviderTotalUsage(t *testing.T) {
	for _, clientGone := range []bool{false, true} {
		t.Run(map[bool]string{false: "upstream EOF", true: "client gone"}[clientGone], func(t *testing.T) {
			...
			common.SetContextKey(c, constant.ContextKeyChannelType, constant.ChannelTypeBaidu)
			if clientGone {
			...
			resp := &http.Response{StatusCode: 200, Header: http.Header{"Content-Type": {"text/event-stream"}}, Body: io.NopCloser(strings.NewReader("data: " + `{"result":"hello","is_end":false,"usage":{"prompt_tokens":10,"total_tokens":14}}` + "\n\n"))}
