...
func TestAliImageHandlerHonorsRequestResponseFormat(t *testing.T) {
	...
	updatedFetchSetting := originalFetchSetting
	updatedFetchSetting.EnableSSRFProtection = false
	system_setting.ReplaceFetchSetting(updatedFetchSetting)
	t.Cleanup(func() {
		system_setting.ReplaceFetchSetting(originalFetchSetting)
	})
