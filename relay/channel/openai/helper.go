...
func HandleFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, lastStreamData string,
	...
	case types.RelayFormatClaude:
		...
		if err := common.Unmarshal(common.StringToByteSlice(lastStreamData), &streamResponse); err != nil {
			...
			sendClaudeFinalStreamResponse(c, info, &streamResponse)
		}
		...
		if info.ClaudeConvertInfo != nil && info.ClaudeConvertInfo.MessageStartSent && !info.ClaudeConvertInfo.Done {
			...
			sendClaudeFinalStreamResponse(c, info, synthetic)
		}
	...
	case types.RelayFormatGemini:
		...
		if err != nil {
			...
			logger.LogLegacyStreamError(c, "error creating Gemini stream state: "+err.Error())
			return
		}
		...
		if err != nil {
			info.StreamSession.Fail("response_conversion_error", err)
			logger.LogLegacyStreamError(c, "error converting final Gemini stream response: "+err.Error())
			return
		}
		if err := sendGeminiStreamResults(c, results); err != nil {
			info.StreamSession.Fail("response_conversion_error", err)
			logger.LogLegacyStreamError(c, "error sending final Gemini stream response: "+err.Error())
			return
		}
		...
		if err != nil {
			info.StreamSession.Fail("response_conversion_error", err)
			logger.LogLegacyStreamError(c, "error finalizing Gemini stream response: "+err.Error())
			return
		}
		if err := sendGeminiStreamResults(c, results); err != nil {
			info.StreamSession.Fail("response_conversion_error", err)
			logger.LogLegacyStreamError(c, "error sending finalized Gemini stream response: "+err.Error())
		}
	}
}

// sendClaudeFinalStreamResponse 转换收尾阶段的 OpenAI 流帧并写出 Claude 事件；转换失败登记为本地转换异常。
func sendClaudeFinalStreamResponse(c *gin.Context, info *relaycommon.RelayInfo, streamResponse *dto.ChatCompletionsStreamResponse) {
	result, err := service.ConvertStreamResponse(c, info, types.RelayFormatClaude, streamResponse)
	if err != nil {
		info.StreamSession.Fail("response_conversion_error", err)
		logger.LogLegacyStreamError(c, "error converting Claude stream response: "+err.Error())
		return
	}
	claudeResponses, ok := result.Value.([]*dto.ClaudeResponse)
	if !ok {
		err := fmt.Errorf("expected Claude stream responses, got %T", result.Value)
		info.StreamSession.Fail("response_conversion_error", err)
