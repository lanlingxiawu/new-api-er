...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	// 统一在此处接管上游响应体的所有权：适配器的 ParseResponse 只读取不关闭。
	// 该 defer 覆盖非 2xx 提前返回、ParseResponse 出错、以及正常成功三条路径。
	// SSE 响应只关闭不排空：排空会一直读到上游流结束。
	defer func() {
...
// (numeric channel-type platform) and by the thirdpartysd2 task plugin. Their
// ResultURL is a credential-gated upstream output URL that clients only reach
// through the content proxy.
func isThirdPartySD2Task(task *model.Task) bool {
	...
	return task.Platform == constant.TaskPlatform(strconv.Itoa(constant.ChannelTypeThirdPartySD2)) ||
		task.Platform == constant.TaskPlatform(model_setting.ThirdPartySD2PluginKey)
}
...
func TaskModel2Dto(task *model.Task) *dto.TaskDto {
	return &dto.TaskDto{
		...
		Data:       task.Data,
		Other:      service.TaskBillingOther(task),
	}
}
