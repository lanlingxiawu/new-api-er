...
var taskPluginKeys = map[constant.TaskPlatform]string{
	...
	// Fork task plugins: tasks persisted by the former Go adaptors keep their
	// numeric channel-type platform and resolve to the replacement plugins.
	constant.TaskPlatform(strconv.Itoa(constant.ChannelTypeXai)):           "xai",
	constant.TaskPlatform(strconv.Itoa(constant.ChannelTypeThirdPartySD2)): "thirdpartysd2",
}
