...
func GetStreamDiagnostic(c *gin.Context) {
	...
	defer cancel()
	diagnostic, err := model.GetStreamDiagnostic(ctx, requestID, createdAt, attempt)
	if err != nil {
		if !errors.Is(err, gorm.ErrRecordNotFound) {
			logger.LogError(c, "Stream diagnostic lookup failed")
		}
