...
func TestGetStatusDoesNotExposePasskeyOrigins(t *testing.T) {
	originalSettings := *system_setting.GetPasskeySettings()
	originalOptionMap := common.OptionMap
	originalServerAddress := system_setting.ServerAddress
	t.Cleanup(func() {
		system_setting.ReplacePasskeySettings(originalSettings)
		common.OptionMap = originalOptionMap
		system_setting.ServerAddress = originalServerAddress
	})
	common.OptionMap = map[string]string{}
	system_setting.ServerAddress = "https://www.example.com"
	system_setting.ReplacePasskeySettings(system_setting.PasskeySettings{Enabled: true, RPID: "example.com"})

	for _, origins := range []string{"https://www.example.com,https://private.example.com", "", "[]"} {
		t.Run("origins="+origins, func(t *testing.T) {
			mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.Origins = origins })
			...
			assert.Equal(t, origins, system_setting.GetPasskeySettings().Origins)
		})
	}
}
...
func TestPasskeyDomainsPreserveCredentialsAcrossVerificationFlows(t *testing.T) {
	for _, kind := range []string{"direct", "login factor", "sensitive action", "historical mixed case"} {
		t.Run(kind, func(t *testing.T) {
			...
			mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.LegacyRPIDs = legacyRPID })
			mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.Origins = "https://example.com,https://www.example.com" })
...
func TestPasskeyDomainsRejectInvalidAssertions(t *testing.T) {
	for _, failure := range []string{"wrong RP ID", "unlisted origin", "origin outside RP scope", "wrong user", "bad signature", "missing user verification", "expired challenge", "removed domain", "known different domain"} {
		t.Run(failure, func(t *testing.T) {
			user, identity := setupSecurityEnrollmentTest(t)
			key := newSecurityLoginPasskey(t, user.Id)
			mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.LegacyRPIDs = "www.example.com" })
			mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.Origins = "https://example.com,https://www.example.com" })
			...
			case "removed domain":
				mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.LegacyRPIDs = "" })
...
func TestPasskeyDomainChoicesRespectOriginAndConfiguration(t *testing.T) {
	...
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) {
		s.RPID, s.LegacyRPIDs, s.Origins, s.AllowInsecureOrigin = "localhost", "", "http://localhost:3000,http://localhost:3001", true
	})
...
func TestPasskeyDomainRemovalWithoutAffectedCredentialsNeedsNoOverride(t *testing.T) {
	user, _ := setupSecurityEnrollmentTest(t)
	setupPasskeyDomainOptions(t)
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.LegacyRPIDs = "www.example.com,WWW.example.com" })
	...
	assert.Equal(t, upper, system_setting.GetPasskeySettings().LegacyRPIDs)
}

func TestPasskeySnapshotDoesNotMaterializeDefaults(t *testing.T) {
	setupSecurityEnrollmentTest(t)
	setupPasskeyDomainOptions(t)
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.RPID, s.Origins = "", "" })
	system_setting.ServerAddress = "https://www.example.com"
	assert.Equal(t, "www.example.com", system_setting.PasskeySettingsSnapshot().EffectiveRPID())
	assert.Empty(t, system_setting.GetPasskeySettings().RPID)
	assert.Empty(t, system_setting.GetPasskeySettings().Origins)
}
...
func TestPasskeyDomainPreviewConfirmationAndAudit(t *testing.T) {
	...
	request := map[string]any{"rp_id": "example.com", "legacy_rp_ids": upper, "origins": system_setting.GetPasskeySettings().Origins, "preview": true}
	...
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.LegacyRPIDs = lower + "," + upper })
...
func TestPasskeyImplicitDomainChangesRetainPreviousTrust(t *testing.T) {
	...
	assert.Equal(t, "https://www.example.com,https://example.com", system_setting.GetPasskeySettings().Origins)
...
func TestPasskeyDefaultsDoNotRestrictUnrelatedServerAddressSetup(t *testing.T) {
	...
	setupPasskeyDomainOptions(t)
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) {
		s.Enabled, s.RPID, s.LegacyRPIDs, s.Origins = false, "", "", ""
	})
...
func TestPasskeyRegistrationRejectsUnavailableRequestOrigin(t *testing.T) {
	_, identity := setupSecurityEnrollmentTest(t)
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.RPID = "www.example.com" })
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.LegacyRPIDs = "example.com" })
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.Origins = "https://example.com,https://www.example.com" })
...
func TestPasskeyRPIDRotationRetainsDefaultsAndRollsBackOnFailure(t *testing.T) {
	setupSecurityEnrollmentTest(t)
	setupPasskeyDomainOptions(t)
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.RPID, s.LegacyRPIDs = "", "" })
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) {
		s.Origins = "https://www.example.com,https://example.com,https://login.example.com"
	})
	system_setting.ServerAddress = "https://www.example.com"
...
func TestPasskeyRegisterFinishRejectsUnapprovedFlowWithoutConsumingIt(t *testing.T) {
	_, identity := setupSecurityEnrollmentTest(t)
	mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.UserVerification = "required" })
	key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	require.NoError(t, err)
