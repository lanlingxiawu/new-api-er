...
// Cases from upstream/main:controller/model_list_test.go. The upstream file's
// DB helpers (setupModelListControllerTestDB / initModelListColumnNames /
// withTieredBillingConfig) live in zz_upstream_test_shims_test.go, where
// setupModelListControllerTestDB additionally restores model.DB / model.LOG_DB
// so the shared harness connection survives these isolated-SQLite tests.
