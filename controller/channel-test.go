...
func testChannel(ctx context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	return testChannelWithGroup(ctx, channel, testUserID, testModel, endpointType, isStream, "")
}

func testChannelWithGroup(ctx context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool, groupOverride string) testResult {
	if ctx == nil {
	...
	_ = model.RecordConsumeLog(c, testUserID, model.RecordConsumeLogParams{
...
func runChannelTestTask(ctx context.Context, mode string, notify bool, report func(processed, total int)) (channelTestSummary, error) {
	...
	// 定时全量测试后顺带刷新已配置的渠道账号余额（该逻辑原挂在旧的 goroutine 定时循环，
	// 渠道测试改为异步系统任务后迁移到此处，避免账号余额自动刷新功能丢失）。
	if allowDisable && (ctx == nil || ctx.Err() == nil) {
		runScheduledChannelAccountBalanceRefresh(updateAllChannelsAccountBalance)
	}
