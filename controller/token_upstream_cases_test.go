...
// Cases from upstream/main:controller/token_test.go. The upstream helpers that
// other carried tests share (tokenAPIResponse, openTokenControllerTestDB,
// migrateTokenControllerTestDB, setupTokenControllerTestDB, seedToken,
// newAuthenticatedContext, decodeAPIResponse) live in
// zz_upstream_test_shims_test.go, where the isolated SQLite handle is swapped
// in and model.DB / model.LOG_DB are restored afterwards. The external-database
// helper below restores the harness handles and database types the same way.
