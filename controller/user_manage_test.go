...
func TestManageUserQuotaRecordsTopupAndAudit(t *testing.T) {
	for _, tc := range []struct {
		...
		{"add", "add", "user.quota_add", "Increased quota of user quota-owner (ID: %d) by 500", 500, 1500},
		{"subtract", "subtract", "user.quota_subtract", "Decreased quota of user quota-owner (ID: %d) by 500", 500, 500},
		{"override_up", "override", "user.quota_override", "Overrode quota of user quota-owner (ID: %d) from 1000 to 1500", 1500, 1500},
		{"override_down", "override", "user.quota_override", "Overrode quota of user quota-owner (ID: %d) from 1000 to 500", 500, 500},
		{"override_unchanged", "override", "user.quota_override", "Overrode quota of user quota-owner (ID: %d) from 1000 to 1000", 1000, 1000},
		{"override_zero", "override", "user.quota_override", "Overrode quota of user quota-owner (ID: %d) from 1000 to 0", 0, 0},
		{"override_negative", "override", "user.quota_override", "Overrode quota of user quota-owner (ID: %d) from 1000 to -1", -1, -1},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, fmt.Sprintf(tc.content, user.Id), logs[0].Content)
