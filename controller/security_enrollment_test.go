...
func setupSecurityEnrollmentTest(t *testing.T) (*model.User, service.AuthIdentity) {
	...
	system_setting.ReplacePasskeySettings(system_setting.PasskeySettings{Enabled: true, RPID: "example.com", Origins: "https://example.com", RPDisplayName: "new-api"})
	t.Cleanup(func() {
...
func TestSecurityEnrollmentAccessTokenMethodPolicy(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			mutatePasskeySettings(func(s *system_setting.PasskeySettings) { s.Enabled = !test.disabledPasskey })
...
func TestSecurityEnrollmentTwoFAFailureAccountingAndStorageErrors(t *testing.T) {
	...
	for _, endpoint := range []struct {
		...
		if endpoint.path == "/api/user/login/2fa" {
			// fork: invalid-parameter errors are localized (Rule 13); default language is English
