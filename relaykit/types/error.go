...
const (
	...
	ErrorCodeRelayTimeout       ErrorCode = "relay_timeout"
...
func (e *NewAPIError) Error() string {
	...
	return kitutil.StripRequestIds(e.Err.Error())
}
...
func (e *NewAPIError) MaskSensitiveError() string {
	...
	return kitutil.StripRequestIds(kitutil.MaskSensitiveInfo(errStr))
}
	...
	if errors.As(err, &newErr) {
		if newErr.RelayError == nil {
			openaiError := OpenAIError{
				Message: kitutil.StripRequestIds(newErr.Error()),
...
func WithOpenAIError(openAIError OpenAIError, statusCode int, ops ...NewAPIErrorOptions) *NewAPIError {
	...
	if len(openAIError.Metadata) > 0 {
		openAIError.Message = kitutil.StripRequestIds(fmt.Sprintf("%s (%s)", openAIError.Message, openAIError.Metadata))
...
func WithClaudeError(claudeError ClaudeError, statusCode int, ops ...NewAPIErrorOptions) *NewAPIError {
	claudeError.Message = kitutil.StripRequestIds(claudeError.Message)
	if claudeError.Type == "" {
...
func errorMessage(err error) string {
	...
	return kitutil.StripRequestIds(err.Error())
}
