...
func TestOpenAIResponsesRequestToClaudeMessagesNilAndModel(t *testing.T) {
	c := newTestGinContext()
	_, err := OpenAIResponsesRequestToClaudeMessages(c, claudeMeta(), nil)
	require.Error(t, err)

	_, err = OpenAIResponsesRequestToClaudeMessages(c, claudeMeta(), &dto.OpenAIResponsesRequest{})
	require.Error(t, err)
...
func TestOpenAIResponsesRequestToClaudeMessagesBasic(t *testing.T) {
	...
	temp := 0.0
	topP := 0.5
	got, err := OpenAIResponsesRequestToClaudeMessages(c, claudeMeta(), &dto.OpenAIResponsesRequest{
		Model:           "claude-test",
		Temperature:     &temp,
		TopP:            &topP,
...
func TestConvertOpenAIResponsesRequestToClaudeMessagesWrapper(t *testing.T) {
	c := newTestGinContext()
	out, err := convertOpenAIResponsesRequestToClaudeMessages(c, claudeMeta(), &dto.OpenAIResponsesRequest{
		Model: "claude-test",
		Input: mustRawMessage(t, "hi"),
	})
	require.NoError(t, err)
