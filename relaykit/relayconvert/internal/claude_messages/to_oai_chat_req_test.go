...
// chanTypeAnthropic 对应主仓库 constant.ChannelTypeAnthropic；relaykit 不引入渠道常量。
const chanTypeAnthropic = 14

func strptr(s string) *string { return &s }

func openRouterInfo(upstreamModel string) convmeta.Meta {
...
func genericInfo(channelType int, upstreamModel, originModel string) convmeta.Meta {
	return &convmeta.Values{
...
func TestReq_ScalarPointersPreserveExplicitZero(t *testing.T) {
	req := dto.ClaudeRequest{
		Model:       "claude-3",
		Temperature: kitutil.GetPointer(0.0),
		MaxTokens:   kitutil.GetPointer(uint(0)),
		TopP:        kitutil.GetPointer(0.0),
		TopK:        kitutil.GetPointer(0),
		Stream:      kitutil.GetPointer(false),
	}
...
func TestReq_NonZeroScalars(t *testing.T) {
	req := dto.ClaudeRequest{
		Model:     "claude-3",
		MaxTokens: kitutil.GetPointer(uint(128)),
		TopP:      kitutil.GetPointer(0.9),
		TopK:      kitutil.GetPointer(40),
		Stream:    kitutil.GetPointer(true),
	}
	out, err := ClaudeMessagesRequestToOpenAIChat(req, nil)
	require.NoError(t, err)
...
func TestReq_OpenRouterThinkingEnabled(t *testing.T) {
	req := dto.ClaudeRequest{
		Model:    "m",
		Thinking: &dto.Thinking{Type: "enabled", BudgetTokens: kitutil.GetPointer(2048)},
	}
	out, err := ClaudeMessagesRequestToOpenAIChat(req, openRouterInfo("anthropic/claude-3"))
	require.NoError(t, err)
...
func TestReq_ThinkingSuffixAppendedWhenOriginHasSuffix(t *testing.T) {
	info := genericInfo(chanTypeAnthropic, "claude-3", "claude-3-thinking")
	req := dto.ClaudeRequest{Model: "claude-3"}
...
func TestReq_ThinkingSuffixNotDoubledWhenModelAlreadyHasSuffix(t *testing.T) {
	info := genericInfo(chanTypeAnthropic, "x", "claude-3-thinking")
...
func TestReq_ThinkingSuffixNotAppliedWhenOriginLacksSuffix(t *testing.T) {
	info := genericInfo(chanTypeAnthropic, "x", "claude-3")
	req := dto.ClaudeRequest{Model: "claude-3"}
	out, err := ClaudeMessagesRequestToOpenAIChat(req, info)
	require.NoError(t, err)
	assert.Equal(t, "claude-3", out.Model)
