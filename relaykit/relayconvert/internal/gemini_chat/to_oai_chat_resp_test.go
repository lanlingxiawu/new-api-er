...
func TestResp_TextCandidate(t *testing.T) {
	...
	assert.Equal(t, types.FinishReasonStop, out.Choices[0].FinishReason)
}
...
func TestResp_FinishReasonMapping(t *testing.T) {
	cases := map[string]string{
		...
		"BLOCKLIST":          types.FinishReasonContentFilter,
		"PROHIBITED_CONTENT": types.FinishReasonContentFilter,
		"SPII":               types.FinishReasonContentFilter,
		"OTHER":              types.FinishReasonContentFilter,
		"SOMETHING_NEW":      types.FinishReasonContentFilter, // default
	}
...
func TestResp_FunctionCallToolCalls(t *testing.T) {
	...
	out := ResponseGeminiChat2OpenAI("i", 1, resp)
	assert.Equal(t, types.FinishReasonToolCalls, out.Choices[0].FinishReason)
...
func TestStream_MaxTokensFinishReason(t *testing.T) {
	...
	assert.Equal(t, types.FinishReasonLength, *out.Choices[0].FinishReason)
}
...
func TestStream_SafetyFinishReason(t *testing.T) {
	for _, reason := range []string{"SAFETY", "RECITATION", "BLOCKLIST", "PROHIBITED_CONTENT", "SPII", "OTHER", "NEW_ONE"} {
		...
		require.NotNil(t, out.Choices[0].FinishReason)
		assert.Equal(t, types.FinishReasonContentFilter, *out.Choices[0].FinishReason, "reason=%s", reason)
	}
}
