...
func StreamResponseOpenAI2Claude(openAIResponse *dto.ChatCompletionsStreamResponse, info convmeta.Meta) []*dto.ClaudeResponse {
	...
	// 两个条件互补，缺一不可：
	//   GetSendResponseCount() == 1 —— info 为 nil（计数为 0）时不下发 message_start；
	//   !state.MessageStartSent    —— 计数在同一次流的多次调用间不递增，仅靠计数会
	//                                 在收尾块上重复下发 message_start。
	// 该标志同时被 relay/channel/openai/helper.go 用来判断流被上游中断时是否需要
	// 补一个 stop 事件。
	if info.GetSendResponseCount() == 1 && !state.MessageStartSent {
