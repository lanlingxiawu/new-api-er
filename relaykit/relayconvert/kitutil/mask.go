...
var requestIdSuffixRegex = regexp.MustCompile(`\s*\(request id:\s*[^)]*\)`)

// StripRequestIds removes trailing "(request id: xxx)" markers from a message.
//
// 上游没有这个函数，本仓库在把上游错误回给客户端前会剥掉请求 ID，避免逐层代理
// 时同一条错误累积出多个 request id。主模块 common.StripRequestIds 是同一份实现，
// 但 relaykit 是独立 module 不能反向依赖主模块，故在此各留一份。
func StripRequestIds(message string) string {
	return strings.TrimSpace(requestIdSuffixRegex.ReplaceAllString(message, ""))
}
