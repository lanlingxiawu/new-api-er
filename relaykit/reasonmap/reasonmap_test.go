...
func TestOpenAIFinishReasonToClaudeStopReason(t *testing.T) {
	cases := []struct {
		...
		{types.FinishReasonContentFilter, "refusal"},
		{"tool_calls", "tool_use"},
		// default: unknown reason passes through unchanged
		{"mystery", "mystery"},
		{"", ""},
...
func TestRoundTrip_StopReason(t *testing.T) {
	...
	assert.Equal(t, types.FinishReasonContentFilter, ClaudeStopReasonToOpenAIFinishReason("refusal"))
	assert.Equal(t, "refusal", OpenAIFinishReasonToClaudeStopReason(types.FinishReasonContentFilter))
