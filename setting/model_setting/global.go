...
var globalSnapshot config.Snapshot[GlobalSettings]

func init() {
	// 注册到全局配置管理器，并登记快照发布函数。
	config.GlobalConfig.RegisterSnapshot("global", &globalSettings, publishGlobalSettings)
}

// publishGlobalSettings 只允许在配置草稿锁内调用（由 RegisterSnapshot 保证）。
func publishGlobalSettings() { globalSnapshot.Publish(globalSettings) }

// GetGlobalSettings 返回不可变快照。通过它写入不会生效——
// 配置变更必须走管理接口，由 ConfigManager 改草稿后重新发布。
func GetGlobalSettings() *GlobalSettings {
	return globalSnapshot.Load()
}

// ReplaceGlobalSettings 整体替换配置并立即重新发布快照（供测试使用）。
func ReplaceGlobalSettings(s GlobalSettings) {
...
func compiledThinkingBlacklist() ([]string, []*regexp.Regexp) {
	entries := GetGlobalSettings().ThinkingModelBlacklist
...
func ShouldPreserveEffortTail(modelName string) bool {
	...
	for _, entry := range GetGlobalSettings().EffortTailModelIDs {
		entry = strings.TrimSpace(entry)
		if entry == "" {
			continue
		}
