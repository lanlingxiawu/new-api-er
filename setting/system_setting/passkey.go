...
func PasskeySettingsSnapshot() PasskeySettings {
	...
	return passkeySnapshot.Load().WithDefaults(serverAddress)
}
...
// GetPasskeySettings 返回存储值的不可变快照，不做 RPID / Origins 推导；
// 需要生效值的调用方使用 PasskeySettingsSnapshot。
func GetPasskeySettings() *PasskeySettings {
	return passkeySnapshot.Load()
}
