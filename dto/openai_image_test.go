...
func TestImageRequest_Rule5_Pointers(t *testing.T) {
	// absent => nil
	var r ImageRequest
	require.NoError(t, common.Unmarshal([]byte(`{"model":"m","prompt":"p"}`), &r))
	assert.Nil(t, r.N)
	...
	var r2 ImageRequest
	require.NoError(t, common.Unmarshal([]byte(`{"model":"m","prompt":"p","n":0,"stream":false,"watermark":false}`), &r2))
