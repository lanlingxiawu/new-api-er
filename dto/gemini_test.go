...
func ginCtxWithRequest(method, target string) *gin.Context {
...
func TestGeminiChatRequest_Unmarshal_SystemInstruction(t *testing.T) {
	...
	require.NoError(t, common.Unmarshal([]byte(`{"contents":[],"systemInstruction":{"role":"system","parts":[{"text":"a"}]}}`), &r))
	require.NotNil(t, r.SystemInstructions)
	assert.Equal(t, "a", r.SystemInstructions.Parts[0].Text)

	// snake_case wins
	var r2 GeminiChatRequest
	require.NoError(t, common.Unmarshal([]byte(`{"contents":[],"system_instruction":{"role":"system","parts":[{"text":"b"}]}}`), &r2))
...
func TestGeminiChatRequest_IsStream(t *testing.T) {
	assert.True(t, (&GeminiChatRequest{}).IsStream(ginCtxWithRequest("POST", "/v1beta/models/x:generateContent?alt=sse")))
	assert.True(t, (&GeminiChatRequest{}).IsStream(ginCtxWithRequest("POST", "/v1beta/models/x:streamGenerateContent")))
	assert.False(t, (&GeminiChatRequest{}).IsStream(ginCtxWithRequest("POST", "/v1beta/models/x:generateContent")))
}
...
func TestGeminiThinkingConfig_Unmarshal(t *testing.T) {
	...
	require.NoError(t, common.Unmarshal([]byte(`{"includeThoughts":true,"thinkingBudget":100,"thinkingLevel":"high"}`), &c))
	...
	require.NoError(t, common.Unmarshal([]byte(`{"include_thoughts":true,"thinking_budget":50,"thinking_level":"low"}`), &c2))
...
func TestGeminiInlineData_Unmarshal(t *testing.T) {
	...
	require.NoError(t, common.Unmarshal([]byte(`{"mimeType":"image/png","data":"AAAA"}`), &d))
	assert.Equal(t, "image/png", d.MimeType)
	assert.Equal(t, "AAAA", d.Data)

	// snake_case wins
	var d2 GeminiInlineData
	require.NoError(t, common.Unmarshal([]byte(`{"mime_type":"audio/wav","data":"BBBB"}`), &d2))
...
func TestGeminiPart_Unmarshal(t *testing.T) {
	...
	var p GeminiPart
	require.NoError(t, common.Unmarshal([]byte(`{"text":"t","inlineData":{"mimeType":"image/png","data":"AAAA"}}`), &p))
	...
	var p2 GeminiPart
	require.NoError(t, common.Unmarshal([]byte(`{"inline_data":{"mime_type":"audio/wav","data":"BBBB"}}`), &p2))
...
func TestGeminiChatGenerationConfig_Unmarshal_CamelCase(t *testing.T) {
	...
	require.NoError(t, common.Unmarshal([]byte(raw), &c))
...
func TestGeminiChatResponse_Unmarshal_WithUsage(t *testing.T) {
	...
	require.NoError(t, common.Unmarshal([]byte(`{"candidates":[],"usageMetadata":{"promptTokenCount":5}}`), &r))
...
func TestGeminiChatResponse_Unmarshal_WithoutUsage(t *testing.T) {
	var r GeminiChatResponse
	require.NoError(t, common.Unmarshal([]byte(`{"candidates":[]}`), &r))
	...
	require.NoError(t, common.Unmarshal([]byte(`{"candidates":[],"usageMetadata":{}}`), &r2))
