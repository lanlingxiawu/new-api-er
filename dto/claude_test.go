...
func TestClaudeRequest_Rule5Pointers(t *testing.T) {
	...
	var r ClaudeRequest
	require.NoError(t, common.Unmarshal([]byte(`{"model":"claude"}`), &r))
	...
	require.NoError(t, common.Unmarshal([]byte(`{"model":"claude","max_tokens":0,"temperature":0,"top_k":0,"stream":false}`), &r2))
