...
func TestAudioRequest_Rule5Speed(t *testing.T) {
	var r AudioRequest
	require.NoError(t, common.Unmarshal([]byte(`{"model":"m","input":"i","voice":"v","speed":0}`), &r))
	require.NotNil(t, r.Speed)
	assert.Equal(t, 0.0, *r.Speed)
	var r2 AudioRequest
	require.NoError(t, common.Unmarshal([]byte(`{"model":"m","input":"i","voice":"v"}`), &r2))
...
func TestEmbeddingRequest_Rule5(t *testing.T) {
	...
	require.NoError(t, common.Unmarshal([]byte(`{"model":"m","input":"i","dimensions":0,"temperature":0}`), &r))
...
func TestRerankRequest_Rule5(t *testing.T) {
	var r RerankRequest
	require.NoError(t, common.Unmarshal([]byte(`{"query":"q","top_n":0,"return_documents":false,"max_chunk_per_doc":0,"overlap_tokens":0}`), &r))
...
// ---------------------------------------------------------------------------
// task.go — TaskResponse.IsSuccess
// ---------------------------------------------------------------------------
