...
func TestOpenAIResponsesResponse_ImageGenerationHelpers(t *testing.T) {
	...
	assert.False(t, empty.HasImageGenerationCall())
	assert.Equal(t, "", empty.GetQuality())
	assert.Equal(t, "", empty.GetSize())

	// output without image gen call
	noImg := &OpenAIResponsesResponse{Output: []ResponsesOutput{{Type: "message"}}}
	assert.False(t, noImg.HasImageGenerationCall())
	assert.Equal(t, "", noImg.GetQuality())
	assert.Equal(t, "", noImg.GetSize())

	// with image generation call
	withImg := &OpenAIResponsesResponse{Output: []ResponsesOutput{
		{Type: "message"},
		{Type: ResponsesOutputTypeImageGenerationCall, Quality: "hd", Size: "1024x1024"},
	}}
	assert.True(t, withImg.HasImageGenerationCall())
	assert.Equal(t, "hd", withImg.GetQuality())
	assert.Equal(t, "1024x1024", withImg.GetSize())
}
