...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.POST("/oauth/email/bind/start", middleware.UserAuth(), middleware.UserCriticalRateLimit("account-security"), middleware.EmailVerificationRateLimit(), middleware.DisableCache(), controller.EmailBindStart)
		apiRouter.POST("/oauth/email/bind/resend", middleware.UserAuth(), middleware.UserCriticalRateLimit("account-security"), middleware.EmailVerificationRateLimit(), middleware.DisableCache(), controller.EmailBindResend)
		...
		apiRouter.POST("/oauth/telegram/bind/start", middleware.UserAuth(), middleware.UserCriticalRateLimit(), middleware.DisableCache(), controller.TelegramLegacyAuth)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.DisableCache(), middleware.UserAuth())
			{
				...
				selfRoute.POST("/token", middleware.UserCriticalRateLimit("access-token"), middleware.DisableCache(), controller.GenerateAccessToken)
				selfRoute.DELETE("/token", middleware.UserCriticalRateLimit("access-token"), middleware.DisableCache(), controller.RevokeAccessToken)
		...
		optionRootRoute.Use(middleware.AdminAuth())
		{
			...
			optionRootRoute.PUT("/passkey/domains", middleware.RequirePermission(authz.SystemSettingsEdit("auth.passkey")), controller.UpdatePasskeyDomains)
			optionRootRoute.GET("/model_pricing", middleware.RequirePermission(authz.SystemSettingsView("billing.model-pricing")), controller.GetModelPricingConfig)
			optionRootRoute.PATCH("/model_pricing", middleware.RequirePermission(authz.SystemSettingsEdit("billing.model-pricing")), controller.UpdateModelPricingConfig)
			optionRootRoute.POST("/model_pricing/convert", middleware.RequirePermission(authz.SystemSettingsEdit("billing.model-pricing")), controller.PreviewModelPricingConversion)
			optionRootRoute.POST("/model_pricing/preview", middleware.RequirePermission(authz.SystemSettingsEdit("billing.model-pricing")), controller.PreviewModelPricing)
