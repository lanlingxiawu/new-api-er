...
- `bench/webui`：图形控制台。网页表单填参数 → 生成/复制命令（省得记长命令），或**直接启动** mock / 压测并把实时输出流式回浏览器；支持在浏览器里保存参数预设。
...
```
go run ./bench/webui        # 然后浏览器打开 http://127.0.0.1:18090
```

- 页面分「Mock 上游」「压测器 loadgen」两栏，loadgen 可切换**闭环基准 / 生产模拟**两种模式，字段随模式自动增减。
- 每栏底部实时显示等价命令，「复制命令」拿去手动跑；「启动」则由控制台拉起子进程（默认 `go run ./bench/mockai` / `go run ./bench/loadgen`，工作目录 `-repo`，默认仓库根），实时输出流式显示在下方日志框，「停止」结束进程。
