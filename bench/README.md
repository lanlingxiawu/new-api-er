...
对 new-api 网关 `/v1/chat/completions` 做端到端压测，真实执行鉴权、渠道分发、relay 转发、token 计数、计费、日志、提成结算全链路。包含两个纯标准库程序：

- `bench/mockai`：OpenAI 兼容 mock 上游。响应时间（TTFB、总时长）与内容（词数、词面）随机；热路径零锁、近零分配，确保 mock 不是瓶颈。
- `bench/loadgen`：压测器。闭环并发，流式/非流式按比例混合，SSE 流完整消费，输出 p50/p90/p95/p99、流式 TTFB、状态码分布与错误采样。
...
1. 管理后台新建渠道：类型 OpenAI，Base URL `http://127.0.0.1:18080`，密钥任意填（mock 不校验），模型填 `gpt-4o-mini`（或 mock `-models` 列表中任意值），分组按需。
...
常用参数：`-c` 并发、`-d` 时长、`-stream-ratio` 流式占比、`-max-tokens`、`-prompt-words`、`-timeout`。
