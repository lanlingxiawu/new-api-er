...
curl --cert certs/client.crt \
  ...
  --cacert certs/ca.crt \
  -X POST https://127.0.0.1:8888/api/v1/heartbeat \
...
curl --cert certs/client.crt \
  --key certs/client.key \
  --cacert certs/ca.crt \
  -X POST https://127.0.0.1:8888/api/v1/accounts/status \
  -H "Content-Type: application/json" \
  -d '{
    "node_name": "node-001",
    "accounts": [
...
curl --cert certs/client.crt \
  --key certs/client.key \
  --cacert certs/ca.crt \
  https://127.0.0.1:8888/api/v1/nodes/127.0.0.1/node-001/accounts
```
