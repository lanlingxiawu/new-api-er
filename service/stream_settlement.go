...
// settleStreamQuota owns the one terminal balance adjustment. On uncertain
// failures it records the intended charge without retrying arithmetic or refund.
// settleStreamQuota 执行受管流式的唯一终止结算，并把结算后的订阅金额刷新到待保存日志。
// 参数 ctx：请求日志上下文；relayInfo：带非 nil StreamResult 的中转信息；params：最终结算参数指针，资金失败时会清零费用与统计项。
// 返回 true 表示本次已尝试结算；false 表示此前已尝试，应跳过后续日志和资金操作。
// 仅采用已记录的资金提交差额；全额退款显式覆盖最终消耗为 0，失败不自动重试非幂等资金运算。
func settleStreamQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, params *ConsumptionSettlementParams) bool {
	stream := relayInfo.StreamResult
