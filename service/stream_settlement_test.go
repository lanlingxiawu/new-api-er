...
func TestClaudeTerminalSettlementOnce(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			info := &relaycommon.RelayInfo{Billing: fixture, UserQuota: 1_000_000_000, StreamResult: &relaycommon.StreamOutcome{}}
			...
			require.Equal(t, tc.quota, info.StreamResult.IntendedQuota)
			require.Equal(t, 100, info.StreamResult.ReservedQuota)
		})
	}
}

// TestClaudeCacheOnlyUsageIsBillable 验证仅受管流式摘要接受纯缓存用量，旧流程及全空摘要维持原有资格。
// 参数 t：当前测试上下文，用于断言、子测试与清理；无返回值，失败通过测试断言报告。
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			info := &relaycommon.RelayInfo{
				...
				StreamResult:                          &relaycommon.StreamOutcome{Failed: true},
			}
