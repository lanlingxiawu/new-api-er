...
// 用户名就会撞唯一索引。
// installForcedUpdateFailure 在 table 上安装 BEFORE UPDATE 触发器，使 id=rowID 的行更新失败并返回 message，
// 用于驱动资金/令牌扣减失败分支；测试结束时自动删除。
// 上游用例写的是 SQLite 触发器语法，本包 TestMain 连接真实 MySQL/PostgreSQL，因此按方言生成等价触发器。
func installForcedUpdateFailure(t *testing.T, table, name string, rowID int, message string) {
	t.Helper()
	switch {
	case common.UsingMainDatabase(common.DatabaseTypePostgreSQL):
		fn := name + "_fn"
		require.NoError(t, model.DB.Exec(fmt.Sprintf(`CREATE OR REPLACE FUNCTION %s() RETURNS trigger AS $$
BEGIN
	IF OLD.id = %d THEN
		RAISE EXCEPTION '%s';
	END IF;
	RETURN NEW;
END;
$$ LANGUAGE plpgsql`, fn, rowID, message)).Error)
		require.NoError(t, model.DB.Exec(fmt.Sprintf(`CREATE TRIGGER %s BEFORE UPDATE ON %s FOR EACH ROW EXECUTE PROCEDURE %s()`, name, table, fn)).Error)
		t.Cleanup(func() {
			model.DB.Exec(fmt.Sprintf("DROP TRIGGER IF EXISTS %s ON %s", name, table))
			model.DB.Exec(fmt.Sprintf("DROP FUNCTION IF EXISTS %s()", fn))
		})
	case common.UsingMainDatabase(common.DatabaseTypeMySQL):
		require.NoError(t, model.DB.Exec(fmt.Sprintf(`CREATE TRIGGER %s BEFORE UPDATE ON %s FOR EACH ROW
BEGIN
	IF OLD.id = %d THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '%s';
	END IF;
END`, name, table, rowID, message)).Error)
		t.Cleanup(func() {
			model.DB.Exec(fmt.Sprintf("DROP TRIGGER IF EXISTS %s", name))
		})
	default:
		require.NoError(t, model.DB.Exec(fmt.Sprintf(`CREATE TRIGGER %s
BEFORE UPDATE ON %s
WHEN OLD.id = %d
BEGIN
...
func TestSettleMidjourneyTaskBillingFundingFailureClearsMarkers(t *testing.T) {
	...
	installForcedUpdateFailure(t, "users", "fail_midjourney_user_update", userID, "forced user quota failure")
...
func TestSettleMidjourneyTaskBillingTokenFailureKeepsFundingRefundable(t *testing.T) {
	...
	installForcedUpdateFailure(t, "tokens", "fail_midjourney_token_update", tokenID, "forced token quota failure")
