...
func updateBatchTasks(ctx context.Context, adaptor BatchTaskPollingAdaptor, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	// 响应体由 ReadAll 读完后排空并关闭，保证连接被归还连接池。
...
func updateVideoSingleTask(ctx context.Context, adaptor TaskPollingAdaptor, ch *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	// try parse as NEXAXIS API response format
	var responseItems taskdto.TaskResponse[model.Task]
	if err = common.Unmarshal(responseBody, &responseItems); err == nil && responseItems.IsSuccess() {
		logger.LogDebug(ctx, "updateVideoSingleTask parsed as NEXAXIS API response format: %+v", responseItems)
	...
	if isDone && snap.Status != task.Status {
		...
		if err != nil {
			...
			logger.LogWarn(ctx, fmt.Sprintf("Task %s CAS lost or no-op update, skip billing", logTaskID))
			shouldFinalizeBilling = false
		}
...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) bool {
	if bc := task.PrivateData.BillingContext; bc != nil && bc.TieredSnapshot != nil {
		...
		// 免费分组的预扣与结算额都是 0，「结算额 ÷ 分组倍率」还原不出上游消耗，
		// 必须把表达式乘分组倍率之前的额度显式交给渠道每日上限。
		upstreamBase := &TaskUpstreamBase{
			Before: bc.TieredSnapshot.EstimatedQuotaBeforeGroup,
			After:  result.ActualQuotaBeforeGroup,
		}
		bc.TieredSnapshot.UsageFacts = usageFacts
		bc.TieredSnapshot.EstimatedTier = result.MatchedTier
		RecalculateTaskQuota(ctx, task, result.ActualQuotaAfterGroup, "任务用量表达式结算", upstreamBase, result.Clamp)
		return true
	}
	...
	if actualQuota := adaptor.AdjustBillingOnComplete(task, taskResult); actualQuota > 0 {
		RecalculateTaskQuota(ctx, task, actualQuota, "adaptor计费调整", nil)
		return true
	}
	// 回退到 token 重算。
