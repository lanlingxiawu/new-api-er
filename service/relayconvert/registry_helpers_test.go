...
// TestMain installs a deterministic media resolver so that any converter that
// needs to resolve inline/base64 media does not error on network access, and
// exercises the exported SetMediaResolver wrapper (media.go).
func TestMain(m *testing.M) {
	gin.SetMode(gin.TestMode)
	SetMediaResolver(MediaResolver{
		GetBase64Data: func(_ *gin.Context, _ types.FileSource, _ ...string) (string, string, error) {
			return "ZGVjb2RlZA==", "image/png", nil
		},
		DecodeBase64FileData: func(_ string) (string, string, error) {
			return "ZGVjb2RlZA==", "image/png", nil
		},
	})
	os.Exit(m.Run())
