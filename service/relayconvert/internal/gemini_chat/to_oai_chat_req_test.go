...
func infoWith(channelType int, upstreamModel string, isStream bool) *relaycommon.RelayInfo {
...
func TestReq_ModelAndStreamFromInfo(t *testing.T) {
	out, err := GeminiGenerateContentRequestToOpenAIChat(&dto.GeminiChatRequest{}, infoWith(constant.ChannelTypeGemini, "gemini-1.5-pro", true))
...
func TestReq_GenerationConfigFull(t *testing.T) {
	req := &dto.GeminiChatRequest{
		GenerationConfig: dto.GeminiChatGenerationConfig{
			...
			TopK:            common.GetPointer(40.0),
			MaxOutputTokens: common.GetPointer(uint(256)),
			CandidateCount:  common.GetPointer(2),
...
func TestReq_GenerationConfigTemperatureZeroPreserved(t *testing.T) {
	...
	req := &dto.GeminiChatRequest{
		GenerationConfig: dto.GeminiChatGenerationConfig{Temperature: common.GetPointer(0.0)},
	}
	out, err := GeminiGenerateContentRequestToOpenAIChat(req, nil)
	require.NoError(t, err)
	require.NotNil(t, out.Temperature)
...
func TestReq_GenerationConfigNonPositiveGuardsDrop(t *testing.T) {
	...
	req := &dto.GeminiChatRequest{
		GenerationConfig: dto.GeminiChatGenerationConfig{
			...
			TopK:            common.GetPointer(0.0),
			MaxOutputTokens: common.GetPointer(uint(0)),
			CandidateCount:  common.GetPointer(0),
		},
	}
	out, err := GeminiGenerateContentRequestToOpenAIChat(req, nil)
	require.NoError(t, err)
