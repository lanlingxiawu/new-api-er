...
func TestClaudeReq_ToolsSchemaTranslation(t *testing.T) {
	...
	got, err := OpenAIChatRequestToClaudeMessages(newGinCtx(), req)
...
func TestClaudeReq_OpenRouterReasoningInvalidJSON(t *testing.T) {
	...
	_, err := OpenAIChatRequestToClaudeMessages(newGinCtx(), req)
	require.Error(t, err)
}
...
func TestClaudeReq_MediaImageAndDocument(t *testing.T) {
	...
	got, err := OpenAIChatRequestToClaudeMessages(newGinCtx(), imgReq)
	require.NoError(t, err)
	content := got.Messages[0].Content.([]dto.ClaudeMediaMessage)
	require.Len(t, content, 2)
	...
	got2, err := OpenAIChatRequestToClaudeMessages(newGinCtx(), pdfReq)
	require.NoError(t, err)
...
func TestClaudeReq_MediaPartNilSourceSkipped(t *testing.T) {
	...
	got, err := OpenAIChatRequestToClaudeMessages(newGinCtx(), dto.GeneralOpenAIRequest{
		Model: "gpt-4",
		Messages: []dto.Message{
			{Role: "user", Content: []any{
